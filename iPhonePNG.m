...
int main(int argc, const char * argv[]) {
    ...
    //id originalPath = [[NSString stringWithCString: argv[1]] stringByExpandingTildeInPath];
    id originalPath = [[NSString stringWithCString:argv[1] encoding:NSUTF8StringEncoding]
                       stringByExpandingTildeInPath];
    id originalFile = [originalPath lastPathComponent];
    if (![originalPath hasPrefix: @"/"])
        originalPath = [[[NSFileManager defaultManager] currentDirectoryPath] stringByAppendingPathComponent: originalPath];

    id newPath = [[originalPath stringByDeletingLastPathComponent] stringByAppendingPathComponent: [NSString stringWithFormat: @"%@-Decoded", [originalFile stringByDeletingPathExtension]]];
    if ([[originalFile pathExtension] length])
        newPath = [newPath stringByAppendingPathExtension: [originalFile pathExtension]];
    
    NSLog(@"Decoding to: %@", [newPath stringByAbbreviatingWithTildeInPath]);
    //[[NSFileManager defaultManager] removeFileAtPath: newPath handler: nil];
    //[[NSFileManager defaultManager] copyPath: originalPath toPath: newPath handler: nil];
    ...
    basePath = (isDirectory) ? newPath : [newPath stringByDeletingLastPathComponent];
    
    if (isDirectory) {
        foreach([[NSFileManager defaultManager] subpathsAtPath: newPath]) {
            id path = [newPath stringByAppendingPathComponent: object];
            if ([@"png" isEqualToString: [[path pathExtension] lowercaseString]])
                convertPath(path);
        }
    } else {
        if ([@"png" isEqualToString: [[newPath pathExtension] lowercaseString]])
            convertPath(newPath);
    }
